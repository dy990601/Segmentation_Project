...
class DEAttention_Module(nn.Module):
    ...
    def forward(self, x, skipx):
        ...
        proj_query = self.query_conv(x).view(m_batchsize, -1, height * width * depth).permute(0, 2, 1)
        proj_key = self.key_conv(x).view(m_batchsize, -1, height * width * depth)
        energy = torch.bmm(proj_query, proj_key)
        attention = self.softmax(energy)
        proj_value = self.value_conv(skipx).view(m_batchsize, -1, height * width * depth)

        out = torch.bmm(proj_value, attention.permute(0, 2, 1))
        out = out.view(m_batchsize, C, height, width, depth)

        out = self.gamma * out + skipx
        return out
...
class DAM_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        m_batchsize, C, height, width, depth = x.size()
        proj_query = self.query_conv(x).view(m_batchsize, -1, depth)
        proj_key = self.key_conv(x).view(m_batchsize, -1, depth).permute(0, 2, 1)
        energy = torch.bmm(proj_query, proj_key)
        attention = self.softmax(energy)
...
class CAM_Module(nn.Module):
    ...
    def forward(self, x):
        """
            inputs :
                x : input feature maps( B * C * H * W * D)
            returns :
                out : attention value + input feature
                attention: B X C X C
        """
...
class UpTransition_1(nn.Module):
    def __init__(self, inChans, outChans, nConvs, elu, dropout=False):
        super(UpTransition_1, self).__init__()
        self.up_conv = nn.ConvTranspose3d(inChans, outChans // 2, kernel_size=2, stride=2,  output_padding=[0,0,1])
...
class UpTransition(nn.Module):
    def __init__(self, inChans, outChans, nConvs, elu, dropout=False):
        ...
        # self.DEAttention = DEAttention_Module(outChans // 2)
    ...
    def forward(self, x, skipx):
        ...
        # skipxdo = self.DEAttention(out, skipxdo)
...
class VNet(nn.Module):
    ...
    def __init__(self, elu=True, nll=False):
        ...
        self.down_tr32 = DownTransition(16, 1, elu, dropout=True)
        ...
        # self.self_att128 = DAM_Module(128)
        # self.self_att64 = DAM_Module(64)
        # self.self_att32 = DAM_Module(32)
        # self.self_att16 = DAM_Module(16)

        # self.self_att128 = DEAttention_Module(128)
        # self.self_att64 = DEAttention_Module(64)
        # self.self_att32 = DEAttention_Module(32)
        # self.self_att16 = DEAttention_Module(16)
    ...
    def forward(self, x):
        ...
        # assert 1>3
        # out128 = self.self_att128(out128)
        out = self.up_tr256(out256, out128)
        # print("out.shape:", out.shape)
        # out64 = self.self_att64(out64)
        out = self.up_tr128(out, out64)
        # print("out:", out.shape)
        # out32 = self.self_att32(out32)
        out = self.up_tr64(out, out32)
        # print("out:", out.shape)
        # assert 1>3
        # out16 = self.self_att16(out16)
...
if __name__ == "__main__":
    ...
    input = torch.randn(1, 1, 32, 256, 256)  # BCDHW
