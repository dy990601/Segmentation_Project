...
def train_epoch(net, loader, optimizer, cost):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        if batch_idx % 10 == 0:
            print(
                "Train Batch {} || Loss: {:.4f} | Training Dice: {:.4f}".format(str(batch_idx).zfill(4), batch_loss.val,
                                                                                dice))
...
def main(args):
    ...
    if args.do_you_wanna_train is True:
        ...
        train_set = MyNifitDataSet(train_list, transforms=trainTransforms, train=True)
        val_set = MyNifitDataSet(val_list, transforms=valTransforms, test=True)
        test_set = MyNifitDataSet(test_list, transforms=valTransforms, test=True)

        train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers=args.num_works,
                                  shuffle=True)  # Here are then fed to the network with a defined batch size
        val_loader = DataLoader(val_set, batch_size=args.batch_size, num_workers=args.num_works, shuffle=False)
        test_loader = DataLoader(test_set, batch_size=args.batch_size, num_workers=args.num_works, shuffle=False)
        ...
        if args.do_you_wanna_load_weights is True:
            net.load_state_dict(torch.load(os.path.join(args.output_path, 'Checkpoint/Best_Dice.pth.gz')))  # load the weights of the network if you have it
        ...
        for epoch in range(args.init_epoch, args.init_epoch + args.num_epoch):  # define the epochs number
            ...
            print("Epoch {} || Time(min): {:.2f}".format(str(epoch).zfill(3), epoch_time / 60))
    ...
    if args.do_you_wanna_check_accuracy is True:
        ...
        net.load_state_dict(torch.load(os.path.join(args.output_path, 'Checkpoint/Best_Dice.pth.gz')))
        ...
        Dice_val = check_accuracy_model(net, val_list, args.resize, args.new_size)

        print("Checking accuracy on testing set")
        Dice_test = check_accuracy_model(net, test_list, args.resize, args.new_size)

        print("Checking accuracy on training set")
        Dice_train = check_accuracy_model(net, train_list, args.resize, args.new_size)
